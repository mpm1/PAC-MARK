...
public class EnvironmentGenerator : MonoBehaviour
{
    ...
    private void GenerateMaze(Vector3Int startLocation, Vector3Int mapMin, Vector3Int mapMax, Vector3Int ghostMin, Vector3Int ghostMax)
    {
        ...
        RemoveWall(startLocation, mapMin, mapMax, walls);

        while(walls.Count > 0)
        {
            int i = Random.Range(0, walls.Count);
            RemoveWall(walls[i], mapMin, mapMax, walls);
            walls.RemoveAt(i);
        }
    }

    private void RemoveWall(Vector3Int location, Vector3Int mapMin, Vector3Int mapMax, List<Vector3Int> walls)
    {
        ...
        if(location.x == mapMin.x || location.y == mapMin.y || location.x == mapMax.x || location.y == mapMax.y)
        {
            checkMax = 2;
        }
